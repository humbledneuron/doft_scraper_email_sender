...
def start_sending_emails(stop_event):
    ...
    def select_file():
        ...
        if file_path:
            ...
            root.destroy()  # Close the program if no file is selected
    ...
    if df is not None:
        num_rows = 10
        max_emails_per_sender = 500  # Set the limit for each sender
        email_counter = 0
        sender_index = 0

        while sender_index < len(sender_details):
            sender_email, sender_pass = sender_details[sender_index]
            for i in range(0, len(df), num_rows):
                if stop_event.is_set():
                    break
                if email_counter >= max_emails_per_sender:
                    sender_index += 1
                    if sender_index >= len(sender_details):
                        break
                    sender_email, sender_pass = sender_details[sender_index]
                    email_counter = 0
        ...
        messagebox.showwarning("Warning", "Failed to load data from selected file.")
