...
send_interval = 4  # Reduced interval between sending emails
scrape_interval = 3  # Reduced interval to execute the scraper script
...
def start_sending_emails(smtp_server, smtp_port):
    ...
    while not stop_event.is_set():
        ...
        def send_email_batch(sender_detail):
            sender_email, sender_pass = sender_detail
            send_email(smtp_server, smtp_port, sender_email, sender_pass, recipient_email, bcc_list, email_subject, email_content)

        with concurrent.futures.ThreadPoolExecutor(max_workers=batch_size) as executor:
            futures = [executor.submit(send_email_batch, sender_detail) for sender_detail in sender_details]
