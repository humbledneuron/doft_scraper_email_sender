...
def start_sending_click():
    if os.name == 'nt':  # Windows
        cli_process = subprocess.Popen(["cmd.exe", "/K", "python", "cli_display.py"], stdin=subprocess.PIPE, text=True, encoding='utf-8', shell=True)
    else:  # macOS/Linux
        cli_process = subprocess.Popen(["xterm", "-hold", "-e", "python3", "cli_display.py"], stdin=subprocess.PIPE, text=True, encoding='utf-8')
...
def main():
    ...
    # recipient_email_list = read_email_list('recipient_email.txt')
    # recipient_email = recipient_email_list[0] if recipient_email_list else ""
