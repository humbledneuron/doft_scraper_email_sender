...
def start_sending_emails(recipient_email, stop_event, cli_process):
    ...
    if df is not None:
        ...
        cli_process.wait()
        messagebox.showinfo("Success", f"All emails have been successfully sent to {recipient_email}")
    else:
...
def start_sending_click():
    ...
    cli_process = subprocess.Popen(["python", "cli_displayer.py"], stdin=subprocess.PIPE, text=True, encoding='utf-8')
