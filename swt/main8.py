...
def start_sending_click():
    if os.name == 'nt':  # Windows
        cli_process = subprocess.Popen(["start", "cmd", "/K", "python", "cli_display.py"], stdin=subprocess.PIPE, text=True, encoding='utf-8', shell=True)
    else:  # macOS/Linux
        cli_process = subprocess.Popen(["xterm", "-e", "python", "cli_display.py"], stdin=subprocess.PIPE, text=True, encoding='utf-8')
